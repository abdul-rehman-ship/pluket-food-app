...
<div className="row">
{
  ...
  {products.map((promotion:any) => (
    <div key={promotion.id} className="border relative border-gray-300 p-4 rounded-lg shadow-lg">
      <img src={promotion.image} alt={promotion.name} className="w-full h-60 object-cover rounded-lg mb-2" />
      <h2 className="text-xl font-semibold mb-2">{promotion.name}</h2>
      <p className="text-gray-600">{promotion.description}</p>
      <div className="absolute top-2 right-2 flex space-x-2">
            <button
              onClick={()=>editPromotion(promotion.id)}
              className="p-2 rounded-full bg-gray-500 text-blue hover:text-white  hover:bg-blue-600"
            >
  ...
  <path d="M21.731 2.269a2.625 2.625 0 00-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 000-3.712zM19.513 8.199l-3.712-3.712-12.15 12.15a5.25 5.25 0 00-1.32 2.214l-.8 2.685a.75.75 0 00.933.933l2.685-.8a5.25 5.25 0 002.214-1.32L19.513 8.2z" />
            ...
            <button onClick={()=>deletePromotion(promotion.id)}
              ...
              className="p-2 rounded-full bg-gray-500 hover:text-white text-red hover:bg-red-600"
            >
             <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
