...

{
  ...
  products.map((product:any)=>{
   return <div key={product.id} className="border relative border-gray-300 p-4 rounded-lg shadow-lg" style={{background:"#8A2BE2FF"}}>
   <img src={product.image}   alt={product.name} className="w-full h-60 object-cover bg-secondary rounded-lg mb-2" />
   <div className="flex items-center justify-between">
  <h2 className="text-xl font-semibold mb-2" style={{ color: "#FFD800FF" }}>
    {product.name}
  </h2>
  <p className="text-xl font-semibold" style={{ color: "#FFD800FF" }}>
    THB {product.price}
  </p>
</div>
   <p className="text-white">{product.description.length>60 ? product.description.slice(0,60)+"...":product.description}</p>
   <div className="absolute top-2 right-2 flex space-x-2" >
         <button
           onClick={()=>editPromotion(product.id)}
...
<path d="M21.731 2.269a2.625 2.625 0 00-3.712 0l-1.157 1.157 3.712 3.712 1.157-1.157a2.625 2.625 0 000-3.712zM19.513 8.199l-3.712-3.712-12.15 12.15a5.25 5.25 0 00-1.32 2.214l-.8 2.685a.75.75 0 00.933.933l2.685-.8a5.25 5.25 0 002.214-1.32L19.513 8.2z" />
         ...
         <button onClick={()=>deletePromotion(product.id)}
